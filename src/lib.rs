//! 自动扩展的数组，由一个可变槽加多个固定槽构成，每个固定槽用不扩容的Vec来装元素，当槽位上的Vec长度不够时，不会扩容Vec，而是线程安全的到下一个槽位分配新Vec。
//! 第一个固定槽位的Vec长度为32。
//! 固定槽迭代性能比Vec慢1-10倍， 主要损失在切换bucket时，原子操作及缓存失效。
//! 在整理时，会一次性将所有固定槽元素移动到可变槽。
...
impl<T: Default> Arr<T> {
    ...
    /// 切分数组，将保留的部分整理到固定槽中，并尝试将当前vec_capacity容量扩容len+additional
    pub fn splice_settle(&mut self, range: Range<usize>, mut len: usize, additional: usize, multiple: usize) {
...
/// vec remain
pub fn vec_remain<T, R: RangeBounds<usize>>(vec: &mut Vec<T>, range: R) 
{
    ...
    unsafe {
        ...
        vec.set_len(vec.len() - start);
        ptr::drop_in_place(s);
        ptr.add(start).copy_to(ptr, vec.len());
    }
}
